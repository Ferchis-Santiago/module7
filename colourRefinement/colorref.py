...
def colour_refinement(graphs):
    ...
    for index, graph in enumerate(graphs):
        # Start assigning colour 1 to all vertices
        colouring = {v: v.degree for v in graph.vertices}
        partitions = max(colouring.values())
        i = 0
    ...
    for index, colouring, iterations, discrete in results:
        ...
        colour_freq_list = sorted(freq_dict.values())
        if len(colour_freq_list) == 1:
            iterations = 0
        key = (colour_sum, discrete, iterations)
        if key not in isomorphic_graphs:
            isomorphic_graphs[key] = ([],colour_freq_list)
...
def basic_colorref(file_path):
    ...
    result = basic_colorref('Benchmark_instances/CrefBenchmark1.grl')
